...
export default function MasjidCard({ masjid, userLocation }: MasjidCardProps) {
  ...
  return (
    <Card className="mb-4 p-6 border-l-4 border-l-teal-400 hover:shadow-lg transition-shadow duration-200">
      {/* Header with name and directions button */}
      <div className="flex items-start justify-between mb-4">
        <h3 className="text-xl font-bold text-gray-900">{masjid.name}</h3>
        <button
      ...
      {masjid.jummha.map((prayerTime, index) => (
        <div key={index} className="mb-4 p-4 bg-gray-50 rounded-lg border-2 border-gray-200 shadow-sm">
          <div className="flex items-center justify-between mb-4">
            <div className="flex items-center text-teal-600">
              <ClockIcon className="w-5 h-5 mr-2" />
              <span className="text-sm font-medium">Prayer Time {index + 1}</span>
            </div>
      ...
      <div className="mt-4 pt-3 border-t border-gray-100">
        <div>
          ...
          <span className="text-sm text-gray-600">{masjid.notes}</span>
        </div>
      </div>
    </Card>
  );
